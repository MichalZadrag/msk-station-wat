package hla13.station.cashRegister;
...
public class CashRegisterAmbassador extends NullFederateAmbassador {
                                    ...
                                    EventRetractionHandle eventRetractionHandle )
    {
        ...
        if(interactionClass == moveToCashRegisterFromDistributorHandle) {
            try {
                int id = EncodingHelpers.decodeInt(theInteraction.getValue(0));
                double time =  convertTime(theTime);
                externalEvents.add(new ExternalEvent(id, ExternalEvent.EventType.MOVE_TO_CASH_REGISTER_FROM_DISTRIBUTOR, time));
                builder.append("Get client from distributor , time=" + time);
